  ...
  return (
    <div>
      <label className="block text-[11px] font-bold mb-2 uppercase tracking-widest"
        style={{ color: focused ? "#22d3ee" : "rgba(226,240,255,0.4)", transition: "color 0.2s" }}>
        {label}
      </label>
      <div className="relative flex items-center rounded-xl"
        style={{
          border: `1px solid ${focused ? "rgba(6,182,212,0.6)" : "rgba(255,255,255,0.08)"}`,
          background: "rgba(255,255,255,0.04)",
          boxShadow: focused ? "0 0 0 3px rgba(6,182,212,0.12), 0 0 20px rgba(6,182,212,0.08)" : "none",
          transition: "border-color 0.2s, box-shadow 0.2s",
        }}>
        <FieldIcon size={14} className="absolute left-4 shrink-0"
          style={{ color: focused ? "#22d3ee" : "rgba(226,240,255,0.3)", transition: "color 0.2s" }} />
        <input type={type} value={value} onChange={onChange} placeholder={placeholder} required={required}
          onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
          className="w-full bg-transparent outline-none text-sm font-medium"
          style={{ color: "#e2f0ff", padding: "14px 42px 14px 42px" }} />
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); setError(d.detail || "Email atau password salah."); setLoading(false); return; }
      const data = await res.json();
      if (data.must_change_password) { setTempToken(data.access_token); setMustChangePass(true); setLoading(false); return; }
      const prof = await fetch(`${API}/auth/me`, { headers: { Authorization: `Bearer ${data.access_token}` } });
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json().catch(() => ({})); setError(d.detail || "Gagal mengganti password."); setLoading(false); return; }
      const data = await res.json();
      const prof = await fetch(`${API}/auth/me`, { headers: { Authorization: `Bearer ${data.access_token}` } });
      const p = await prof.json();
      setAuth({ name: p.full_name, email: p.email, role: p.role }, data.access_token);
  ...
  return (
    <div style={{
      ...
      padding: "16px", background: "#020408", overflow: "hidden"
    }}>
      <style>{`
      ...
      <div style={{
        ...
        boxShadow: "0 32px 80px rgba(0,0,0,0.9), 0 0 0 1px rgba(6,182,212,0.08), 0 0 60px rgba(6,182,212,0.06)",
        opacity: visible ? 1 : 0,
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: "32px" }}>
          ...
          <h1 style={{
            ...
            letterSpacing: "-0.02em", color: "#e2f0ff",
            opacity: visible ? 1 : 0,
          ...
          }}>CamMatrix — Panel Administrator</p>
        </div>
        {error && (
          <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <InputField type="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder="admin@vms.com" label="Email" icon={Mail} required />
          <InputField type={showPass ? "text" : "password"} value={password}
            onChange={e => setPassword(e.target.value)} placeholder="••••••••" label="Password" icon={Lock} required
            rightEl={
              <button type="button" onClick={() => setShowPass(!showPass)}
                style={{ color: "rgba(226,240,255,0.3)", background: "none", border: "none", cursor: "pointer", padding: 4 }}>
          ...
          <button type="submit" id="login-submit-btn" disabled={loading}
            ...
            {loading
              ? <><span className="btn-spinner" />Masuk...</>
              : <>Masuk <ArrowRight size={15} /></>}
          </button>
        </form>
        <div style={{ margin: "20px 0", display: "flex", alignItems: "center", gap: "12px" }}>
          <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />
          <span style={{ fontSize: "11px", color: "rgba(226,240,255,0.25)" }}>atau</span>
          <div style={{ flex: 1, height: "1px", background: "rgba(255,255,255,0.06)" }} />
        </div>
        <Link to="/live" id="login-live-link"
          style={{
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.background = "linear-gradient(145deg, rgba(6,182,212,0.08), rgba(6,182,212,0.02))";
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
              <span style={{ fontSize: "14px", fontWeight: 700, color: "#e2f0ff", lineHeight: 1 }}>Akses Publik</span>
              <span style={{ fontSize: "11px", color: "rgba(226,240,255,0.4)", lineHeight: 1 }}>Siaran langsung (tanpa login)</span>
            </div>
          </div>
          <ArrowRight size={16} style={{ color: "rgba(226,240,255,0.3)" }} />
        </Link>
      </div>
      {mustChangePass && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
              ...
              <div>
                <h2 style={{ fontSize: "14px", fontWeight: 700, color: "#e2f0ff", margin: 0 }}>Wajib Ganti Password</h2>
                <p style={{ fontSize: "11px", color: "rgba(226,240,255,0.4)", margin: 0 }}>Demi keamanan akun Anda</p>
              </div>
            </div>
            <form onSubmit={handleChangePassword} style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              <InputField type={showNewPass ? "text" : "password"} value={newPassword}
                onChange={e => setNewPassword(e.target.value)} placeholder="Minimal 8 karakter"
                label="Password Baru" icon={Lock} required
                rightEl={
              ...
              <button type="submit" disabled={loading}
                style={{
                  ...
                  fontSize: "14px", fontWeight: 700, color: "#020408", border: "none", cursor: "pointer",
