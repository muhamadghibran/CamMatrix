...
const statusCfg = {
  ...
  offline:   { color: "#6b7280", label: "offline",   bg: "rgba(107,114,128,0.1)", border: "rgba(107,114,128,0.2)" },
...
function CameraModal({ onClose, onSave, editData }) {
  ...
  const fields = [
    { label: t("cameras.modal.name"),     key: "name",     placeholder: t("cameras.modal.namePH") },
    { label: t("cameras.modal.location"), key: "location", placeholder: t("cameras.modal.locationPH") },
    ...
    { label: t("cameras.modal.password"), key: "password", placeholder: "••••••••", type: "password" },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.65)", backdropFilter: "blur(8px)" }}>
      <div className="w-full max-w-md rounded-2xl overflow-hidden animate-card-enter" style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-card-border)", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: "1px solid var(--color-card-border)" }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg,#06b6d4,#00ffff)", boxShadow: "0 0 16px rgba(6,182,212,0.4)" }}>
              <Camera size={14} className="text-white" />
            </div>
            <div>
              <h3 className="text-sm font-bold" style={{ color: "var(--color-text-base)" }}>{isEdit ? "Edit Kamera" : t("cameras.modal.title")}</h3>
              <p className="text-[11px]" style={{ color: "var(--color-text-sub)" }}>{isEdit ? `Mengedit: ${editData.name}` : "Tambahkan kamera RTSP baru"}</p>
            </div>
          </div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ color: "var(--color-text-sub)", backgroundColor: "var(--color-surface-elevated)", transition: "background-color 0.15s" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(239,68,68,0.1)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "var(--color-surface-elevated)")}>
            <X size={14} />
          </button>
        </div>
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            {fields.map(({ label, key, placeholder, type = "text" }) => (
              <div key={key} className={key === "name" || key === "ip" ? "col-span-2" : ""}>
                <label className="block text-[11px] font-semibold mb-1.5 uppercase tracking-wider" style={{ color: "var(--color-text-sub)" }}>{label}</label>
                <input
        ...
        <div className="flex gap-3 px-6 pb-6">
          ...
          <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white flex items-center justify-center gap-2"
            style={{ background: "linear-gradient(135deg,#06b6d4,#00ffff)", boxShadow: "0 4px 14px rgba(6,182,212,0.45)", transition: "transform 0.15s, box-shadow 0.15s" }}
            onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(6,182,212,0.55)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "0 4px 14px rgba(6,182,212,0.45)"; }}>
            <Check size={15} /> {isEdit ? "Simpan Perubahan" : t("cameras.modal.add")}
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="w-full max-w-xs rounded-2xl overflow-hidden animate-card-enter" style={{ backgroundColor: "var(--color-surface)", border: "1px solid rgba(239,68,68,0.3)", boxShadow: "0 32px 80px rgba(0,0,0,0.5)" }}>
        <div className="p-6 text-center space-y-4">
          ...
          <div className="flex gap-3">
            <button onClick={onCancel} className="flex-1 py-2 rounded-xl text-sm font-semibold" style={{ border: "1px solid var(--color-card-border)", color: "var(--color-text-sub)" }}>
              Batal
            </button>
            <button onClick={onConfirm} className="flex-1 py-2 rounded-xl text-sm font-bold text-white" style={{ background: "linear-gradient(135deg,#ef4444,#dc2626)", boxShadow: "0 4px 14px rgba(239,68,68,0.4)" }}>
              Hapus
            </button>
          </div>
        </div>
...
export default function CamerasPage() {
  ...
  return (
    <div className="space-y-6 relative z-10 w-full">
      ...
      {deleteTarget && <ConfirmModal message={`Yakin ingin menghapus kamera "${deleteTarget.name}"?`} onConfirm={handleDelete} onCancel={() => setDeleteTarget(null)} />}

      {/* Status pills */}
      <div className="flex items-center gap-3 animate-card-enter opacity-0-init">
        {[
          { label: `${counts.live} Live`,       color: "#10b981" },
          { label: `${counts.recording} Rekam`, color: "#ef4444" },
          { label: `${counts.offline} Offline`, color: "#6b7280" },
        ].map(({ label, color }) => (
          <span key={label} className="flex items-center gap-1.5 text-[11px] font-bold px-3 py-1.5 rounded-full" style={{ color, backgroundColor: `${color}12`, border: `1px solid ${color}28` }}>
      ...
      {/* Toolbar */}
      <div className="flex items-center justify-between gap-4 animate-card-enter opacity-0-init delay-100">
        ...
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white"
          style={{ background: "linear-gradient(135deg,#06b6d4,#00ffff)", boxShadow: "0 4px 14px rgba(6,182,212,0.4)", transition: "transform 0.2s ease, box-shadow 0.2s ease" }}
          onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 6px 20px rgba(6,182,212,0.55)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = ""; e.currentTarget.style.boxShadow = "0 4px 14px rgba(6,182,212,0.4)"; }}>
          <Plus size={16} /> {t("cameras.addCamera")}
        </button>
      </div>

      {/* Table */}
      <div className="rounded-[18px] overflow-hidden animate-card-enter opacity-0-init delay-200" style={{ backgroundColor: "var(--color-surface)", border: "1px solid var(--color-card-border)", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-sm" style={{ color: "var(--color-text-sub)" }}>Tidak ada kamera ditemukan.</td></tr>
            ) : filtered.map((cam, i) => {
              const s = statusCfg[cam.status];
              const isHov = hovered === cam.id;
              return (
                <tr key={cam.id} className="animate-card-enter opacity-0-init"
                  style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--color-card-border)" : "none", animationDelay: `${300 + i * 60}ms`, backgroundColor: isHov ? "var(--color-surface-elevated)" : "", transition: "background-color 0.15s ease" }}
                  onMouseEnter={() => setHovered(cam.id)} onMouseLeave={() => setHovered(null)}
                >
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${s.color}10`, border: `1px solid ${s.color}22` }}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      {cam.status === "offline" ? <WifiOff size={12} style={{ color: s.color }} /> : <Wifi size={12} style={{ color: s.color }} />}
                      <span className="text-[11px] font-bold px-2.5 py-1 rounded-lg" style={{ backgroundColor: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => setEditCamera(cam)} className="p-2 rounded-xl"
                        style={{ color: "#06b6d4", backgroundColor: isHov ? "rgba(6,182,212,0.1)" : "transparent", transition: "background-color 0.15s, transform 0.15s" }}
                        onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.1)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.transform = ""; }}>
                        <Edit2 size={14} />
                      </button>
                      <button onClick={() => setDeleteTarget(cam)} className="p-2 rounded-xl"
                        style={{ color: "#ef4444", backgroundColor: isHov ? "rgba(239,68,68,0.1)" : "transparent", transition: "background-color 0.15s, transform 0.15s" }}
                        onMouseEnter={(e) => { e.currentTarget.style.transform = "scale(1.1)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.transform = ""; }}>
                        <Trash2 size={14} />
                      </button>
