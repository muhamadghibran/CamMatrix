    ...
    const draw = (ts) => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        if (!isLowEnd) {
          ...
          if (dx * dx + dy * dy < 8100) { p.x -= dx * 0.009; p.y -= dy * 0.009; }
        }
        ctx.beginPath();
  ...
  return (
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", overflow: "hidden" }}>
      <div className="animate-orb" style={{
        position: "absolute", width: 650, height: 650, top: "-20%", left: "-12%",
        borderRadius: "50%",
        background: "radial-gradient(circle, rgba(6,182,212,0.12) 0%, rgba(6,182,212,0.03) 45%, transparent 70%)",
        filter: `blur(${blur})`, willChange: "transform",
      }} />
      <div className="animate-orb-alt" style={{
        position: "absolute", width: 550, height: 550, top: "-5%", right: "-10%",
        borderRadius: "50%",
        background: "radial-gradient(circle, rgba(139,92,246,0.09) 0%, rgba(99,102,241,0.03) 45%, transparent 70%)",
        filter: `blur(${blur})`, willChange: "transform",
      }} />
      {!isLowEnd && (
        <div className="animate-orb" style={{
          position: "absolute", width: 400, height: 400, bottom: "5%", left: "35%",
          borderRadius: "50%",
          background: "radial-gradient(circle, rgba(6,182,212,0.06) 0%, transparent 65%)",
          filter: "blur(45px)", animationDelay: "7s", willChange: "transform",
        }} />
      )}
    </div>
  );
